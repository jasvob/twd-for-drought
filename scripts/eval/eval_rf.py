...
arg_parser = argparse.ArgumentParser(
    prog='Evaluate Random Forest Regressor',
    description='Evaluate random forest regressor on the test set and save results to file.',
)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader_test):
            ...
            tgt_fcast = targets[:, -forecast_len:, :]

            rf_inputs = torch.cat([inputs[:, :backcast_len, :].reshape(inputs.shape[0], -1), tgt_bcast.reshape(tgt_bcast.shape[0], -1), inputs[:, -forecast_len:, :].reshape(inputs.shape[0], -1)], dim=-1)
            rf_targets = tgt_fcast.reshape(tgt_fcast.shape[0], -1)

            start = time.time()
            fcast = model.predict(rf_inputs.numpy())
            inf_times.append(time.time() - start)
            fcast = torch.from_numpy(fcast.reshape(tgt_fcast.shape[0], tgt_fcast.shape[1], -1))
            ...
            fcast = dataset_test.unnormalize_data(fcast, dataset_test.normalization[2], norm_targets)
